...
Future<int> runInteractiveCodemod2(
  ...
  String? changesRequiredOutput,
    ...
    runInteractiveCodemodSequence2(
      filePaths,
      [suggestor],
      args: args,
...
///     );
Future<int> runInteractiveCodemodSequence2(
  Iterable<String> filePaths,
  ...
  try {
    ...
    return overrideAnsiOutput<Future<int>>(
        ...
        () => _runInteractiveCodemod2(filePaths, suggestors, parsedArgs,
            defaultYes: defaultYes,
            changesRequiredOutput: changesRequiredOutput));
...
Future<int> _runInteractiveCodemod2(Iterable<String> filePaths,
  ...
  if (filePaths.isEmpty) {
    logger.fine('codemod found no files');
    return ExitCode.success.code;
  }
  ...
  logger.info('done');
  final patchesForLater = <SourcePatch>[];
  ...
  if (failOnChanges) {
    if (numChanges > 0) {
      patchesForLater.forEach((SourcePatch patch) {
        stdout.write(terminalClear());
        stdout.write(patch.renderRange());
        stdout.writeln();
      ...
      logger.warning('$numChanges change(s) needed.');
