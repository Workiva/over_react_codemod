...
ProcessResult runUpgrade({String onDirectory}) {
  ...
  return Process.runSync('pub', ['run', 'over_react_codemod:react16_upgrade'], workingDirectory: onDirectory);
}

main() {
  group('React1_16_upgrade', () {
    test('exits with a status of 1 when there is a comment', () {
      final result = runUpgrade(onDirectory: 'test/executables/test_components/file_with_comment');
    ...
    group('exits with a status of 0 when', () {
      test('there is no comment', () {
        final result = runUpgrade(onDirectory: 'test/executables/test_component'
